...
from repositories.Ingredient import Ingredient
app = Flask(__name__)
...
def scale(calo, pro, fat, satfat, fiber, carb):
    with app.app_context():
        ...
        raw_data = [
            [item['name'], item['nu_grams'], item['nu_calories'], item['nu_proteins'], item['nu_fats'], item['nu_sat_fats'], item['nu_fibers'], item['nu_carbs'], item['nu_price']]
        ...
        data = pd.DataFrame(raw_data, columns=["Food", "Grams", "Calories", "Protein", "Fat", "Sat.Fat", "Fiber", "Carbs", "Cost"])

        # Tách label và các thông số
        data0 = data.iloc[:, 2:8].values  # Dinh dưỡng từ Calories đến Carbs
        data1 = data.iloc[:, 0].values     # Tên thực phẩm
        ...
        print("Top 10 giải pháp tối ưu:")
        for cost, foods in food_results[:10]:
            print(f"Tổng giá: {cost:.2f} Dollars")
            for food in foods:
                print(f"  - {food[0]}: Trọng lượng = {food[1]:.2f} g, Giá = {food[2]:.2f} Dollars")
            print()  # Dòng trống giữa các giải pháp
        return food_results[:10]
